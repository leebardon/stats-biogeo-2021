...
with alive_bar(3) as bar:
    ...
    cocco_cut, darwin_cocco_cut, cocco_summary = SizeTestAnalysis.pres_abs_summary(
        cocco_predictions, darwin, CELLS, "cocco",
    )
    diatom_cut, darwin_diatom_cut, diatom_summary = SizeTestAnalysis.pres_abs_summary(
        diatom_predictions, darwin, CELLS, "diatom",
    )
    bar()
    t
    (
